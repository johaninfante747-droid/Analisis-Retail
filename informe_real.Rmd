...
```
<div align="center"> <span style="font-size: 10pt;">*Figura 1, fuente: elaboracion propia a partir de referencias web.*</span> </div>
...
```
<div align="center"> <span style="font-size: 10pt;">*Figura 2, fuente: elaboracion propia a partir de referencias web.*</span> </div>
\subsection{Análisis de Estacionalidad (Series de Tiempo Simples)}
...
El análisis se fundamentó en un conjunto de datos estructurado (denominado computacionalmente como DataFrame), el cual contiene el registro histórico de las transacciones. Para garantizar la viabilidad del análisis estadístico, se aplicaron técnicas de limpieza y transformación de datos (Data Wrangling), destacando las siguientes operaciones metodológicas:

\begin{itemize}
\item{Transformación temporal: Se convirtió la variable de la fecha de compra a un formato estandarizado de series de tiempo (Datetime) para poder aislar y agrupar los datos según la estacionalidad mensual.}
...
\section{Análisis de la Estructura Demográfica}
...
```{r, fig.align='center', out.width="50%"}
knitr::include_graphics("Imagenes/Piramide.jpg")
```
Al observar la gráfica, se evidencia que el grueso de los consumidores se concentra en el rango de 26 a 35 años, destacando una participación equitativa entre hombres y mujeres, alcanzando aproximadamente 100 transacciones por género en este segmento. Por el contrario, el grupo de menores de 25 años representa la minoría absoluta de la clientela, lo que sugiere que las estrategias de marketing actuales de la tienda no están captando la atención de la generación más joven, teniendo mayor resonancia con un público adulto.

\section{Correlación de Variables Numéricas}

Para evaluar estadísticamente las dinámicas de compra y comprobar empíricamente si el perfil demográfico determina el volumen de gasto, se ejecutó una matriz de correlación visualizada mediante un mapa de calor (Heatmap). Esta herramienta permite medir la fuerza y dirección de la relación lineal entre las variables transaccionales clave del año 2023.

```{r, fig.align='center', out.width="50%"}
knitr::include_graphics("Imagenes/Matriz.jpg")
```
El análisis de la matriz revela un hallazgo fundamental para el negocio: la variable Edad (Age) presenta una correlación prácticamente nula con el monto total gastado (Total Amount, reflejando un coeficiente de -0.06). Esto indica que la edad del consumidor no es un factor predictivo sobre su nivel de gasto. Por el contrario, el ingreso total está fuertemente impulsado por el Precio por Unidad (Price per Unit), el cual muestra una alta correlación positiva de 0.85, indicando que las ventas de la tienda dependen más de artículos de alto valor que del volumen masivo de artículos económicos (cuya correlación con la cantidad es de apenas 0.37).

\section{Estacionalidad y Flujo de Ingresos Mensuales}

El seguimiento temporal de las ventas es un componente vital del análisis exploratorio, ya que permite identificar los picos de demanda y las fluctuaciones estacionales a lo largo del año 2023. Esta información resulta estratégica para la optimización de inventarios y la planificación de futuras campañas de marketing. En la siguiente figura se ilustra la evolución de los ingresos totales consolidados mensualmente.

```{r, fig.align='center', out.width="50%"}
knitr::include_graphics("Imagenes/Lineas.jpg")
```
El análisis temporal revela una alta volatilidad en los ingresos a lo largo del año, caracterizada por fluctuaciones abruptas en lugar de una tendencia de crecimiento estable. El pico máximo de ventas se registró durante el mes 5 (mayo), superando los $50,000 en ingresos, seguido de repuntes secundarios significativos en febrero (mes 2) y octubre (mes 10). Por el contrario, el mes 9 (septiembre) representó una caída crítica para el negocio, registrando el nivel de ingresos más bajo de todo el periodo (por debajo de los 25,000). Este comportamiento cíclico sugiere que el negocio depende de temporadas muy específicas, lo que exige estrategias de inventario ágiles para los meses de alta demanda y promociones más agresivas para mitigar las caídas en el tercer trimestre.
...
```{r, fig.align='center', out.width="50%"}
knitr::include_graphics("Imagenes/Barras.jpg")
```
El cruce de variables revela que el segmento de 46 a 55 años es el que registra el mayor volumen total de transacciones, impulsado significativamente por una alta demanda en la categoría de Electrónica (Electronics). Por otro lado, la categoría de Ropa (Clothing) mantiene una demanda sólida y constante, predominando fuertemente en los consumidores mayores de 36 años. Finalmente, los artículos de Belleza (Beauty) presentan un consumo estable y transversal en todas las edades, sin llegar a dominar de forma absoluta ningún segmento demográfico. Estos resultados sugieren estratégicamente que las campañas de marketing para productos tecnológicos de alto valor deberían focalizarse en el público de mediana edad, desmitificando la idea de que la electrónica es exclusiva de los segmentos más jóvenes.

\chapter{Conclusiones y Recomendaciones}

A partir del Análisis Exploratorio de Datos (EDA) aplicado a los registros transaccionales del sector retail durante el año 2023, se derivan las siguientes conclusiones fundamentales:

\textbf{1.-} El análisis demostró empíricamente que la edad del consumidor no tiene un impacto predictivo sobre su nivel de gasto (r = -0.06), siendo los ingresos impulsados principalmente por la venta de artículos de alto valor unitario (r = 0.85).

\textbf{Recomendación} Fomentar el upselling. Se sugiere capacitar a la fuerza de ventas e implementar algoritmos de recomendación que incentiven la compra de productos Premium, maximizando el valor del ticket promedio.

\textbf{2.-} Existe una brecha generacional en la captación. Aunque el volumen demográfico principal se concentra en adultos jóvenes (26 a 35 años), el negocio no está logrando penetrar en la generación más joven (<25 años).

\textbf{Recomendación} Diseñar campañas de captación para la Generación más joven, utilizando estrategias de inbound marketing y programas de fidelización orientados específicamente a este segmento de mercado actualmente desaprovechado.

\textbf{3.-} Contrario a los supuestos tradicionales, el segmento entre 46 y 55 años resultó ser el motor transaccional más fuerte del año, impulsado abrumadoramente por la adquisición de productos de Electrónica.

\textbf{Recomendación} Redirigir y focalizar la pauta publicitaria de los artículos tecnológicos hacia los consumidores de 46 a 55 años, utilizando canales y mensajes acordes a su poder adquisitivo en lugar de enfocarse solo en el público juvenil.

\textbf{4.-} La rentabilidad del negocio presenta una alta vulnerabilidad estacional, con una dependencia crítica del mes pico (mayo) y una preocupante caída de ingresos en el tercer trimestre (septiembre).

\textbf{Recomendación} Implementar estrategias de inventario ágil, garantizando el máximo abastecimiento entre abril y mayo, y estructurar campañas de descuentos agresivos para septiembre con el fin de mitigar las caídas de demanda.
